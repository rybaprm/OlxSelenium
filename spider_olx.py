...
class OlxSpider():
    ...
    def parse(self):
        ...
        for olx_ad in self.browser.find_elements_by_xpath(
                '//*[@id="offers_table"]/tbody/tr[@class="wrap"]'):
            ad_url = olx_ad.find_element_by_xpath(
                './/*/div/h3/a').get_attribute('href') # find URL of ad
            ad_text = olx_ad.find_element_by_xpath(
                './/*/div/h3/a/strong').text # find text of ad
            ad_date = olx_ad.find_element_by_xpath(
                './/*/div[@class ="space rel"]/p/small[2]/span').text # date
            ad_price = olx_ad.find_element_by_xpath(
                './/*/div/p[@class="price"]/strong').text.strip('$').\
                replace(' ','') # find price of ad
    ...
    def parse_pages(self):
        ...
        while True:
            ...
            next_page = self.browser.find_elements_by_xpath(
                '//*/span[@class="fbold next abs large"]/a') # find next page
            # check for next page
            if next_page:
